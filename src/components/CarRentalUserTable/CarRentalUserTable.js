...
import {InputTextArea, InputTextarea }from 'primereact/inputtextarea'
import { InputText } from 'primereact/inputtext';
import { ApiCapUser, CarRentalDetails, CarRentalUpdate } from '../../services/apiCapRegister/apiCapRegister';
...
const CarRentalUserTable = (props) => {
    let emptyProduct = {
        _id: '',
        name: '',
        email: '',
    ...
    useEffect(() => {
        CarRentalDetails().then(res => {
          
            const data = res.data.userData.reverse()
            const cabData=res.data.carData
            console.log(res)
    ...
    const saveProduct = () => {
        ...
        if (isUpdated.length === 0 || isUpdatedCab.length===0) {
            ...
            const cabDetails={
                ...
                fuelType:eachCabDetail.fuelType,
                seats: eachCabDetail.seats,
                location:eachCabDetail.carLocation,
                about:_eachCabDetail.about,
                pricePerHrs:eachCabDetail.pricePerHrs,
                features:eachCabDetail.features,
                images:eachCabDetail.images, 
                extraHrsCharges:eachCabDetail.extraHrsCharges,
                purchasedYear:eachCabDetail.purchasedYear,
                fuelCapacity:eachCabDetail.fuelCapacity
            }
            const data = {
                personalDetails: personalDetails,
                cabDetails: cabDetails
            }
            CarRentalUpdate(data).then(res => {
    ...
    const editProduct = (id) => {
        ...
        const cData=cabDatas.filter(each=>(
          ...
          setFromlocation(cData[0].carLocation)
          setProduct(...data);
            ...
            {value => {
                ...
                return (
                    ...
                    <div className="datatable-cab-crud-demo" data-testid="CabUserTable">
                        ...
                        <Dialog  visible={productDialog}  header="Cab User Details"  style={{ width: '450px' }}  modal className="p-fluid " footer={productDialogFooter} onHide={hideDialog}>
                            <div className="field ">
                          ...
                          <div className="field">
                            <label  htmlFor="priceKm" >Price Per Hours</label>
                            <div className="formGrid">
                              <InputNumber style={{height:'40px'}} value={eachCabDetail.pricePerHrs} placeholder='In Rupees' onChange={(e) => onInputNumberChange(e, 'pricePerHrs')} />
                            </div>
                          </div>
                          {/* For Fuel Capacity */}
                   
                            <div className="field">
                              <label  htmlFor="acCharges" >Fuel Capacity</label>
                              <div className="formGrid">
                                <InputNumber  style={{height:'40px'}} value={eachCabDetail.fuelCapacity} placeholder='In Rupees' onChange={(e) => onInputNumberChange(e, 'fuelCapacity')} />
                              </div>
                            </div>
                             {/* For Fuel Capacity */}
                   
                             <div className="field">
                              <label  htmlFor="acCharges" >About</label>
                              <div className="formGrid">
                                <InputTextarea  style={{height:'80px'}} value={eachCabDetail.about} placeholder="comments about Your car" onChange={(e) => onInputChange(e, 'about')} />
                              </div>
                            </div>
                        
                          {/* Additional Charges */}
                          <div className="field">
                            <label htmlFor="extraKm" >Extra Hour Charges</label>
                            <div className="form-grid">
                              <InputNumber style={{height:'40px'}} value={eachCabDetail.extraHrsCharges} placeholder='In Rupees' onChange={(e) => onInputNumberChange(e, 'extraHrsCharges')} />
                            </div>
                          </div>
                        </Dialog>
...
CarRentalUserTable.propTypes = {};

CarRentalUserTable.defaultProps = {};

export default CarRentalUserTable;
